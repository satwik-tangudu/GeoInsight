...
def render_chart(data_json):
    ...
    # Read the custom choropleth map script (adjust path as necessary)
    with open("src/assets/chart8.js") as f:
        script = f.read()

    # Inject D3.js and custom script into Streamlit component
    components.html(
        f"""
        <div id="choropleth-map" style="width:100%; height:600px;"></div>
        <script>{d3_script}</script>
        <script>
            {script}
            drawChoroplethMap(`{data_json}`);
        </script>
        """,
        height=600,
    )